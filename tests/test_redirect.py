...
class TestRedirect:
    ...
    def test_redirect_from_account_through_constructs(self):
        driver = webdriver.Chrome()
        random = Random()
        random_number = str(random.randint(1000, 9999))
        email = f'getmanyulia{random_number}@yandex.ru'
        ...
        WebDriverWait(driver, 3).until(
            expected_conditions.visibility_of_element_located((By.XPATH, './/a[text()="Профиль"]')))
        driver.find_element(*Locator.BUTTON_HEADER_CONSTRUCT).click()
        WebDriverWait(driver, 3).until(
            expected_conditions.visibility_of_element_located((By.XPATH, './/p[text()="Личный Кабинет"]')))
        assert driver.find_element(By.XPATH, './/h1[text()="Соберите бургер"]').text == 'Соберите бургер'
        driver.quit()

    def test_redirect_from_account_through_logo(self):
        driver = webdriver.Chrome()
        random = Random()
        random_number = str(random.randint(1000, 9999))
        email = f'getmanyulia{random_number}@yandex.ru'
...
test = TestRedirect()
test.test_redirect_from_account_through_logo()
